            ...
            if move
                ...
                loop do
                    ...
                    if grid[tile_y][tile_x] == '?'
                        candidates = queue.select do |item|
                            item_x, item_y = to_coords(grid, item)
                            item_distance = (item_x - unit_x).abs + (item_y - unit_y).abs
                            tile_distance = (tile_x - unit_x).abs + (tile_y - unit_y).abs
                            item_distance == tile_distance
                        end
                        pp ['!', tile, candidates] if candidates.size > 0 && tile > candidates.min
                        # find next step
                        # puts '***'
                        loop do
                            ancestor = previous[tile]
